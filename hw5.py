...
class QuestionnaireAnalysis:
    ...
    def show_age_distrib(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for index, value in enumerate(bins):
            if index == len(bins)-1:
                break
            count.append(np.count_nonzero((ages>=value)&(ages<bins[index + 1])))
        hist = np.array(count)
    ...
    def remove_rows_without_mail(self) -> pd.DataFrame:
        ...
        def conditions_for_mail(email: str):
            if (email.count("@") != 1) | (email.find("@") == 0) | (email.find("@") == len(email)-1):
                ...
                return False
            #elif (email[-4::] != '.com'):
            #    return False # add condition that needs to end in .com / add condition that no special characters exist after last '.'
            else:
                return True
    ...
    def score_subjects(self, maximal_nans_per_sub: int = 1) -> pd.DataFrame:
        ...
        question_df = self.data.loc[:,"q1":"q5"]
        score = question_df.mean(axis=1)
        index_na = question_df.count(axis=1) < (5 - maximal_nans_per_sub)
        score[index_na] = np.nan
